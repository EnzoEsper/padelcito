...
1.  **Discover Dashboard (Geo-Matchmaking):** A sleek feed showing nearby available matches. It must prominently feature a configurable radius slider, category filter chips, a list/map segmented toggle, match cards displaying missing players (e.g., "3/4 Players filled"), skill level badges, time/date, and location distance. **Map mode:** a contained rounded map card (not full-bleed), category-colored pins with cluster bubbles when zoomed out, a synced bottom carousel of compact match cards, recenter and "Search this area" controls. Production uses `react-native-maps`; JSX mocks in `docs/design-references/` are illustrative only.
